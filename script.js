...
function sortByReviews(array) {
	const _goods = [];
	array.forEach(obj => {
		_goods.push(JSON.parse(JSON.stringify(obj)));
	});

	_goods.sort((a, b) => {
		return (toNumber(a.ratingRevievs) - toNumber(b.ratingRevievs));
	});
...
console.log(sortByReviews(goods));
console.log('---');
console.log(`Sort by new price:`);
console.log(sortByPrice(goods));